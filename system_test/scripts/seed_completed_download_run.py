#!/usr/bin/env python3
"""
Inject a completed benchmark run from a JSON fixture into the E2E/test database.

Loads system_test/fixtures/e2e_run_ac1.json (run AC1 from moonshot.db run id 3) and
inserts all related rows with FK remapping. Writes .e2e-download-run.json for Playwright.

Usage:
  MOONSHOT_DB_PATH=... PYTHONPATH=moonshot_core python system_test/scripts/seed_completed_download_run.py
"""
...
DEFAULT_RUN_NAME = "AC1"
DEFAULT_FIXTURE_PATH = REPO_ROOT / "system_test" / "fixtures" / "e2e_run_ac1.json"
DEFAULT_MANIFEST_PATH = REPO_ROOT / "system_test" / ".e2e-download-run.json"
...
def seed_completed_download_run(
    *,
    ...
    """
    Inject the fixture run into MOONSHOT_DB_PATH and return manifest metadata.
    """
    fixture_path = fixture_path or Path(
        os.environ.get("E2E_RUN_FIXTURE_PATH", str(DEFAULT_FIXTURE_PATH))
    )
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        f"Seeded completed run {manifest['runName']!r} "
