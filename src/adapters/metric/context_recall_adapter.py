...
class ContextRecallAdapter(MetricPort):

    ERROR_INIT_MSG = "[ContextRecallAdapter] There was an error initializing the ContextRecallAdapter: {}"
    ERROR_RETRIEVING_CONNECTORS_MSG = (
        "[ContextRecallAdapter] There was an error retrieving metric connectors: {}"
    )
    ERROR_EVALUATING_RESULT_MSG = (
        "[ContextRecallAdapter] There was an error evaluating the individual result: {}"
    )
    ERROR_RETRIEVING_RESULTS_MSG = (
        "[ContextRecallAdapter] There was an error retrieving results: {}"
    )
    NO_CONNECTOR_AVAILABLE_MSG = (
        "[ContextRecallAdapter] No metric connector available for evaluation."
    )
    FAILED_MODEL_PREDICTIONS_MSG = "[ContextRecallAdapter] Failed to get model predictions from the evaluation model."
    LOADING_CONNECTOR_MSG = "[ContextRecallAdapter] Loading connector with model '{model}' and adapter '{adapter}'"
    SUCCESSFULLY_LOADED_CONNECTORS_MSG = (
        "[ContextRecallAdapter] Successfully loaded all metric connectors."
    )
    ...
    async def get_individual_result(self, entity: MetricIndividualEntity) -> dict:
        ...
        score = await scorer.single_turn_ascore(data_sample)
    ...
    async def get_results(self, entities: list[MetricIndividualEntity]) -> dict:
        ...
        This method calculates the context recall score based on the evaluated results
        ...
        Returns:
            dict: A dictionary containing the context recall score for the context recall metric.
