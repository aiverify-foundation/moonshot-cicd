...
class FaithfulnessAdapter(MetricPort):

    ERROR_INIT_MSG = "[FaithfulnessAdapter] There was an error initializing the FaithfulnessAdapter: {}"
    ERROR_RETRIEVING_CONNECTORS_MSG = (
        "[FaithfulnessAdapter] There was an error retrieving metric connectors: {}"
    )
    ERROR_EVALUATING_RESULT_MSG = (
        "[FaithfulnessAdapter] There was an error evaluating the individual result: {}"
    )
    ERROR_RETRIEVING_RESULTS_MSG = (
        "[FaithfulnessAdapter] There was an error retrieving results: {}"
    )
    NO_CONNECTOR_AVAILABLE_MSG = (
        "[FaithfulnessAdapter] No metric connector available for evaluation."
    )
    FAILED_MODEL_PREDICTIONS_MSG = "[FaithfulnessAdapter] Failed to get model predictions from the evaluation model."
    LOADING_CONNECTOR_MSG = "[FaithfulnessAdapter] Loading connector with model '{model}' and adapter '{adapter}'"
    SUCCESSFULLY_LOADED_CONNECTORS_MSG = (
        "[FaithfulnessAdapter] Successfully loaded all metric connectors."
    )
    ...
    async def get_results(self, entities: list[MetricIndividualEntity]) -> dict:
        ...
        This method calculates the faithfulness score based on the evaluated results
        ...
        Returns:
            dict: A dictionary containing the faithfulness score for the faithfulness metric.
