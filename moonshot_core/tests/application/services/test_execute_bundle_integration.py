...
def test_db_path():
    ...
    db_path = (
        moonshot_core_root / "data" / "database" / "moonshot_pytest_execute_bundle.db"
    )
    if db_path.exists():
        db_path.unlink()
    return str(db_path)
...
def test_execute_bundle_happy_path_with_db(
    ...
    mock_connector.get_response = AsyncMock(
        return_value=ConnectorResponseEntity(
            response="execute_bundle db test response", context=[]
        )
    )
    ...
    _assert_run_completed(
        session_manager, run_id, test_ids, "execute_bundle db test response"
    )


def _assert_run_completed(
    session_manager, run_id: int, test_ids: list, expected_prediction: str
):
    """Assert all tests for run_id are completed with prompt results."""
    for test_id in test_ids:
        run_test_id, status, end_dt = _get_run_test_status(
            session_manager, run_id, test_id
        )
        assert (
            run_test_id is not None
        ), f"run_test_status missing for run_id={run_id}, test_id={test_id}"
...
def test_execute_bundle_duplicate_run_name_no_new_run(
    ...
    with (
        ...
        assert (
            run_after_first.id == run_id_created
        ...
        assert (
            run_after_second.id == run_id_created
...
def test_get_benchmark_run_prompts_api(
    ...
    session_manager = SessionManager.get_instance()
    run_id = _insert_benchmark_run(session_manager, "integration-get-prompts-api")
    BenchmarkRunTestBundlePopulationService().populate_run_bundle(
        run_id, "minimal-bundle"
    )
    ...
    _assert_run_completed(session_manager, run_id, test_ids, "get-prompts-api response")
...
def test_get_benchmark_run_results_api(
    ...
    session_manager = SessionManager.get_instance()
    run_id = _insert_benchmark_run(session_manager, "integration-get-results-api")
    BenchmarkRunTestBundlePopulationService().populate_run_bundle(
        run_id, "minimal-bundle"
    )
    ...
    _assert_run_completed(session_manager, run_id, test_ids, "get-results-api response")
    ...
    margin_by_tid = {
        m["test_id"]: m["margin_of_error"] for m in margins if "test_id" in m
    }
    ...
    run_status = data.get("test_run_status") or []
    assert isinstance(run_status, list)
    assert len(run_status) >= len(test_ids)
    status_by_tid = {row["test_id"]: row for row in run_status}
    for tid in test_ids:
        assert tid in status_by_tid
        assert status_by_tid[tid].get("start_dt") is not None


@pytest.mark.integration
def test_patch_benchmark_run_prompt_user_feedback_api(
    shared_config_seed_service,
    test_db_env,
    tmp_path,
):
    """
    PATCH /api/benchmark-run-test-prompts/{prompt_id} updates user_evaluation and user_notes.
    """
    assert CONFIG_PATH_TWO_TESTS.exists(), f"Fixture missing: {CONFIG_PATH_TWO_TESTS}"
    shared_config_seed_service.seed_if_test_file_changed(
        config_path=CONFIG_PATH_TWO_TESTS
    ...
    session_manager = SessionManager.get_instance()
    run_id = _insert_benchmark_run(session_manager, "integration-patch-prompt-api")
    BenchmarkRunTestBundlePopulationService().populate_run_bundle(
        run_id, "minimal-bundle"
    )
    ...
    mock_connector.get_response = AsyncMock(
        return_value=ConnectorResponseEntity(
            response="patch-prompt-api response", context=[]
        )
    )
    mock_connector.configure = MagicMock()
    mock_metric = MagicMock()
    ...
    _assert_run_completed(
        session_manager, run_id, test_ids, "patch-prompt-api response"
    )

    client = TestClient(fastapi_app)
    res_get = client.get(f"/api/benchmark-runs/{run_id}/results")
    assert res_get.status_code == 200, res_get.text
    p0 = res_get.json()["prompts"][0]
    prompt_row_id = p0["id"]
    assert prompt_row_id is not None

    res_patch = client.patch(
        f"/api/benchmark-run-test-prompts/{prompt_row_id}",
        json={"user_evaluation": 0, "user_notes": "reviewer note"},
    )
    assert res_patch.status_code == 200, res_patch.text
    body = res_patch.json()
    assert body["user_evaluation"] == 0
    assert body["user_notes"] == "reviewer note"
    assert body["id"] == prompt_row_id
    ...
    with session_manager.get_session() as session:
        ...
        assert row.user_evaluation == 0
        assert row.user_notes == "reviewer note"
...
def test_get_benchmark_run_test_bundles_api_after_execute(
    ...
    session_manager = SessionManager.get_instance()
    run_id = _insert_benchmark_run(session_manager, "integration-run-test-bundles-api")
    BenchmarkRunTestBundlePopulationService().populate_run_bundle(
        run_id, "minimal-bundle"
    )
