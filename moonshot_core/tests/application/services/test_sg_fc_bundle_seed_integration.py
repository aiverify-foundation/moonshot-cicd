"""
Integration test: full shared.yaml seed includes the SG FC bundle
(13 Singapore facts-in-Singapore tests / datasets).
...
EXPECTED_SG_FC_DATASETS = sorted(
    [
        "sg_fc_economy",
        "sg_fc_environment",
        "sg_fc_law_legal",
        "sg_fc_policy_digital",
        "sg_fc_policy_education",
        "sg_fc_policy_environment",
        "sg_fc_policy_health",
        "sg_fc_policy_housing",
        "sg_fc_policy_national_security",
        "sg_fc_policy_social_support_schemes",
        "sg_fc_policy_transport",
        "sg_fc_political",
        "sg_fc_social",
    ]
)
...
def sg_fc_test_db_path():
    db_path = (
        MOONSHOT_CORE_ROOT / "data" / "database" / "moonshot_pytest_sg_fc_bundle.db"
    )
    if db_path.exists():
        db_path.unlink()
    return str(db_path)
...
@pytest.fixture(scope="session")
def sg_fc_test_db_env(sg_fc_test_db_path):
    old_val = os.environ.get("MOONSHOT_DB_PATH")
    os.environ["MOONSHOT_DB_PATH"] = sg_fc_test_db_path
...
@pytest.fixture(scope="session")
def sg_fc_shared_config_seed_service(sg_fc_test_db_env):
...
def test_sg_fc_bundle_seeds_thirteen_tests_with_expected_datasets(
    sg_fc_shared_config_seed_service,
    sg_fc_test_db_env,
):
    assert SHARED_CONFIG_PATH.exists(), f"Missing shared config: {SHARED_CONFIG_PATH}"

    sg_fc_shared_config_seed_service.seed_if_test_file_changed(
        config_path=SHARED_CONFIG_PATH
    )

    config_adapter = BenchmarkTestConfigAdapter()
    bundle_db_id = config_adapter.get_bundle_id_by_system_name_latest("sg-fc")
    test_ids = config_adapter.get_test_ids_by_bundle_id(bundle_db_id)

    assert len(test_ids) == 13, f"Expected 13 tests, got {len(test_ids)}: {test_ids}"

    datasets_in_bundle = sorted(
        {config_adapter.get_test_info(tid)[1] for tid in test_ids}
    )
    assert datasets_in_bundle == EXPECTED_SG_FC_DATASETS

    for tid in test_ids:
        _test_name, _ds, metric_name = config_adapter.get_test_info(tid)
        assert metric_name == "sg_fact_check_adapter"

    dataset_repo = SqlAlchemyDatasetRepository()
    for system_name in EXPECTED_SG_FC_DATASETS:
