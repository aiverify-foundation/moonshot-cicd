...
class TestSharedConfigSeedService:
    ...
    def test_seed_from_data_creates_bundle_and_test_and_grouping(
        ...
        grouping_calls = mock_adapter.insert_grouping.call_args_list
        assert grouping_calls[0][1]["test_bundle_id"] == 1
        assert grouping_calls[0][1]["test_id"] == 10
        assert grouping_calls[1][1]["test_bundle_id"] == 1
        assert grouping_calls[1][1]["test_id"] == 11
