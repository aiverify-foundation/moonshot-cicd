"""
Integration test: full shared.yaml seed includes MOON-622 Undesirable Content bundle
(12 MLCommons AILuminate tests / datasets).
...
EXPECTED_UNDESIRABLE_CONTENT_DATASETS = sorted(
    [
        "mlc-ailuminate-cse",
        "mlc-ailuminate-hte",
        "mlc-ailuminate-iwp",
...
def undesirable_content_test_db_path():
    db_path = (
        MOONSHOT_CORE_ROOT
        / "data"
        / "database"
        / "moonshot_pytest_undesirable_content_bundle.db"
    )
    if db_path.exists():
        db_path.unlink()
    return str(db_path)
...
@pytest.fixture(scope="session")
def undesirable_content_test_db_env(undesirable_content_test_db_path):
    old_val = os.environ.get("MOONSHOT_DB_PATH")
    os.environ["MOONSHOT_DB_PATH"] = undesirable_content_test_db_path
...
@pytest.fixture(scope="session")
def undesirable_content_shared_config_seed_service(undesirable_content_test_db_env):
...
def test_undesirable_content_bundle_seeds_twelve_tests_with_expected_datasets(
    undesirable_content_shared_config_seed_service,
    undesirable_content_test_db_env,
):
    assert SHARED_CONFIG_PATH.exists(), f"Missing shared config: {SHARED_CONFIG_PATH}"

    undesirable_content_shared_config_seed_service.seed_if_test_file_changed(
    ...
    assert len(test_ids) == 12, f"Expected 12 tests, got {len(test_ids)}: {test_ids}"

    datasets_in_bundle = sorted(
        {config_adapter.get_test_info(tid)[1] for tid in test_ids}
    )
    assert datasets_in_bundle == EXPECTED_UNDESIRABLE_CONTENT_DATASETS
    ...
    for system_name in EXPECTED_UNDESIRABLE_CONTENT_DATASETS:
        dataset_id = config_adapter.get_dataset_id_by_system_name_latest(
            system_name
        )
        prompts = dataset_repo.get_prompts_by_dataset_id(dataset_id)
        assert len(prompts) >= 1, f"Dataset {system_name!r} should have at least one prompt"
