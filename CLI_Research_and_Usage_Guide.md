...
## Overview
This document provides a comprehensive analysis of the `cli()` function in `moonshot_be/src/entrypoints/cli/cli.py` and various methods to run the Moonshot application.
...
- **Usage**: `moonshot` or `python -m moonshot_be`
...
```bash
# Navigate to the moonshot_be directory
cd /Users/kokbaisheng/code/moonshot-cicd/moonshot_be

# Install dependencies (first time only)
poetry install

# Run the main CLI (welcome screen)
poetry run python -m moonshot_be
